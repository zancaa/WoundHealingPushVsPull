#ifndef TESTCONTINUALGROWTHMODEL_HPP_
#define TESTCONTINUALGROWTHMODEL_HPP_
...
#include "OffLatticeSimulationWithLargeTissueStoppingEvent.hpp" //Stop simulation if there are too many cells
...
class TestContinualGrowthModel : public AbstractCellBasedTestSuite
{
private:

    void SetFilename(std::string& filename, double force, double qvf, int rep)
    {
        ...
        filename = "WoundHealing/ContinualGrowth/LeadingEdgeForce_"
...
public:
    /*
     * Wounded epidermal skin model with continual growth of proliferative hub.
     */
    void TestContinualGrowthProliferativeHubModel()
    {
        ...
        for (unsigned i=0; i<p_mesh->GetNumElements(); i++)
        {
            ...
            ContactInhibitionUniformCellCycleModel* p_model = new ContactInhibitionUniformCellCycleModel();
        ...
        */
        ...
        std::stringstream repAsString;
        repAsString << seed;
        std::string output_directory = "WoundHealing/ContinualGrowth/InitialConditions/Run_" + repAsString.str(); ;
        ...
        for (unsigned index = 0; index<=factive_num_sweeps; index ++)     
        {
            for (unsigned index2 = 0; index2<=mqvf_num_sweeps; index2 ++)
            {
                    ...
                    ++elem_iter)
                {
                    ...
                    c_vector<double, 2> centroid_of_cell = p_cell_population->rGetMesh().GetCentroidOfElement(elem_index);
                    double y = centroid_of_cell[1];

                    // Set up leading edge
                    if ( !(elem_iter->IsElementOnBoundary() && (y > 2.0)) )
                    {
                        p_cell->SetCellProliferativeType(p_transit_type);
                    }
                    // For each node we create a cell with our cell-cycle model. 
                    ContactInhibitionUniformCellCycleModel* p_cc_model = static_cast<ContactInhibitionUniformCellCycleModel*>(p_cell->GetCellCycleModel());
                ...
                p_simulator->AddForce(p_nh_force);
                ...
                //Set output directory
                std::string filename;
                SetFilename(filename, force_val, qvf_val, seed);
...
#endif /* TESTCONTINUALGROWTHMODEL_HPP_ */
